...
def cnn_modified(x_, drop):
    var_list = []
    conv1 = tf.layers.conv2d(   inputs=x_,
                                filters=15,  # number of filters
                                kernel_size=[5, 5],
                                padding="same",
                                name = 'conv1',
    ...
    pool_flat = tf.contrib.layers.flatten(pool2, scope='pool3flat') 
    dense = tf.layers.dense(inputs=pool_flat, units=500, activation=tf.nn.relu)
    
    dropout = tf.layers.dropout(inputs=dense, rate = 0.5, training = drop )
    
    for scope_name in ['conv1','conv2','dense']:            
...
def apply_classification_loss(model_function):
    ...
    model_dict = {'graph': g,'inputs':[x_, y_, drop], 'train_op': train_op,
